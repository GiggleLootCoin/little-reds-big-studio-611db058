...
function routesFor(logicalId: LogicalId): RouteCandidate[] {
  ...
  return FREE_RUNNERS
    .filter((runner) => runner.kind === "public" && runner.capabilities.some((capability) => wanted.has(capability)))
...
function markHealthy(logicalId: string, space: string) {
  ...
  state.successes += 1; state.failures = 0; state.nextRetryAt = 0; state.lastSuccessAt = Date.now(); state.lastError = undefined;
}
function markUnhealthy(logicalId: string, space: string, error: unknown) {
  const state = getHealth(logicalId, space);
  state.failures += 1; state.lastFailureAt = Date.now(); state.lastError = error instanceof Error ? error.message : String(error);
...
export function connectFreeSpace(space: string) {
  ...
  if (!client) { client = Client.connect(space) as unknown as Promise<GradioClient>; client.catch(() => clients.delete(space)); clients.set(space, client); }
  return client;
}
export function freeFile(file: FileLike): FileLike { return file; }

function isUrlLike(text: string) { return /^(https?:|blob:|data:|\/file=|file=)/.test(text); }
export function outputUrl(value: unknown): string | null {
  ...
  if (typeof value === "string") {
    ...
    if (isUrlLike(text)) return text;
    try { if (text.startsWith("{") || text.startsWith("[")) return outputUrl(JSON.parse(text)); } catch { return null; }
    return null;
  }
  ...
  for (const field of ["url", "blob", "path", "data", "value", "audio", "image", "video", "audio_url", "video_url", "image_url", "file"]) { const found = outputUrl(item[field]); if (found) return found; }
  return null;
}
...
function textOutput(value: unknown): string | null {
  if (typeof value === "string" && value.trim()) return value.trim();
  if (Array.isArray(value)) for (const item of value) { const found = textOutput(item); if (found) return found; }
  if (value && typeof value === "object") { const record = value as Record<string, unknown>; for (const field of ["text", "transcript", "transcription", "value", "data"]) { const found = textOutput(record[field]); if (found) return found; } }
  return null;
}
...
const aliases: Record<string, string[]> = {
  audio: ["audio", "inputaudio", "sourceaudio", "sourceaudiopath", "audiofile", "input"], inputaudio: ["audio", "inputaudio", "sourceaudio"], sourceaudio: ["sourceaudio", "audio", "inputaudio"],
  image: ["image", "inputimage", "sourceimage", "imagefile", "input"], inputimage: ["image", "inputimage", "sourceimage"],
  prompt: ["prompt", "description", "textprompt", "styleprompt", "text"], description: ["description", "prompt", "textprompt"], textprompt: ["textprompt", "description", "prompt", "text"],
  text: ["text", "targettext", "prompt", "lyrics"], targettext: ["targettext", "text"], lyrics: ["lyrics", "lrc", "text"], lrc: ["lrc", "lyrics"],
  seed: ["seed"], randomizeseed: ["randomizeseed"], steps: ["steps", "inferstep", "inferencesteps", "diffusionsteps"], cfgstrength: ["cfgstrength", "guidancescale"], guidance: ["guidance", "guidancescale"], guidancescale: ["guidancescale", "guidance"], height: ["height"], width: ["width"],
  refaudio: ["refaudio", "referenceaudio", "sourceaudio"], referenceaudio: ["referenceaudio", "refaudio", "sourceaudio"], reftext: ["reftext", "referencetext"], language: ["language"], speaker: ["speaker", "voice", "voiceid"], instruct: ["instruct", "instruction"], instruction: ["instruction", "instruct"],
...
function endpointScore(logicalId: LogicalId, endpoint: string, spec: EndpointInfo, inputs: Inputs) {
  if (Array.isArray(inputs)) return endpoint === "/predict" ? 5 : 0;
  const record = inputs as Record<string, unknown>; const name = normalizeName(endpoint);
  const keywords: Record<LogicalId, string[]> = {
    speechToText: ["transcrib", "whisper", "asr", "speech"], music: ["music", "song", "infermusic", "generate"], image: ["image", "generate", "txt2img"], video: ["video", "generate", "infer"],
    voiceClone: ["clone", "voice", "custom", "generate"], voicePreset: ["custom", "voice", "generate"], voiceSwap: ["convert", "voice", "rvc", "seed"], vocalSeparation: ["separat", "stem", "demuc", "vocal"],
  };
  let score = keywords[logicalId].some((keyword) => name.includes(keyword)) ? 20 : 0;
  for (const param of spec.parameters ?? []) {
    const paramName = parameterName(param);
    if (pickValue(paramName, record, logicalId) !== undefined || param.default !== undefined || param.optional) score += 5; else score -= 30;
  }
  return score;
}
...
async function apiFor(space: string, forceFresh = false) {
  ...
  const info = await withTimeout(client.view_api(true), API_TIMEOUT); apiCache.set(space, { info, expires: Date.now() + API_CACHE_TTL }); return info;
}
async function resolveEndpoint(logicalId: LogicalId, route: RouteCandidate, inputs: Inputs, preferred?: string, forceFresh = false) {
  const client = await withTimeout(connectFreeSpace(route.space), CONNECT_TIMEOUT); const info = await apiFor(route.space, forceFresh); const map = endpointMap(info);
  ...
  const ranked = Object.entries(map).map(([endpoint, spec]) => ({ endpoint, spec, score: endpointScore(logicalId, endpoint, spec, record) })).filter(({ spec }) => (spec.parameters ?? []).every((param) => { const name = parameterName(param); return pickValue(name, record, logicalId) !== undefined || param.default !== undefined || param.optional; })).sort((a, b) => b.score - a.score);
  const selected = ranked[0]; if (!selected || selected.score < 0) throw new Error(`No compatible live endpoint is exposed by ${route.space}.`);
...
async function normalizeInput(value: unknown): Promise<unknown> {
  ...
  if (value && typeof value === "object") { const entries = await Promise.all(Object.entries(value as Record<string, unknown>).map(async ([key, item]) => [key, await normalizeInput(item)] as const)); return Object.fromEntries(entries); }
  return value;
}
function buildInputs(logicalId: LogicalId, spec: EndpointInfo, inputs: Inputs): unknown {
  ...
  return (spec.parameters ?? []).map((param) => { const name = parameterName(param); const value = pickValue(name, record, logicalId); if (value !== undefined) return value; if (param.default !== undefined) return param.default; if (param.optional) return null; throw new Error(`Required input "${param.parameter_name ?? param.label ?? "unknown"}" is unavailable.`); });
}
async function execute(client: GradioClient, endpoint: string, spec: EndpointInfo, logicalId: LogicalId, inputs: Inputs) {
  ...
  if (client.predict) { try { return await withTimeout(client.predict(endpoint, normalized), JOB_TIMEOUT); } catch (error) { console.warn(`[Buddy] predict failed for ${endpoint}; trying queued execution`, error); } }
  const job = client.submit(endpoint, normalized); let latest: unknown = null; for await (const message of job) if (message.type === "data") latest = message.data ?? null;
  if (latest == null) throw new Error("The generation service returned no result."); return latest;
}
function usableResult(value: unknown, logicalId: LogicalId) { const result = firstOutput(value); return logicalId === "speechToText" ? Boolean(textOutput(result)) : Boolean(outputUrl(result)); }
...
async function collect(logicalId: LogicalId, inputs: Inputs, onStatus?: (message: string) => void, preferred?: string) {
  const candidates = routesFor(logicalId).sort((a, b) => { const aState = getHealth(logicalId, a.space); const bState = getHealth(logicalId, b.space); const aPenalty = aState.nextRetryAt > Date.now() ? 1_000_000 : aState.failures * 10; const bPenalty = bState.nextRetryAt > Date.now() ? 1_000_000 : bState.failures * 10; return (b.priority - bPenalty) - (a.priority - aPenalty); });
  if (!candidates.length) throw new Error(`No free public route is configured for ${logicalId}.`); let lastError: unknown = null;
...
function logicalIdFor(space: string): LogicalId | "" {
  ...
  const found = (Object.keys(FREE_SPACE_IDS) as LogicalId[]).find((logicalId) => routesFor(logicalId).some((route) => route.space === space)); return found ?? "";
}
