  ...
  return (
    <Panel eyebrow="Mix core" title="Red's QRange" icon={<SlidersHorizontal className="size-5" />} defaultOpen>
      ...
      <div className="flex flex-wrap gap-2">
        <Chip>Local settings</Chip><Chip>Session-safe</Chip><Chip>No API</Chip>
      </div>
      <StudioButton className="w-full" onClick={() => { setStudio({ qrange: { ...q } }); setSaved(true); window.setTimeout(() => setSaved(false), 1600); }}>
    ...
    <Panel eyebrow="Local files" title="Audio, Voice & File Uploads" icon={<UploadCloud className="size-5" />}>
      ...
      {files.length > 0 && (
        <div className="space-y-2">
          {files.map((file) => (
            <div key={`${file.name}-${file.url}`} className="rounded-xl border border-border bg-background/50 p-3">
