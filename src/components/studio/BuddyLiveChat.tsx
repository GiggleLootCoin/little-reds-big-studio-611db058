...
export function BuddyLiveChat() {
  ...
  const [status, setStatus] = useState("Buddy is ready. Turn on Live Voice for hands-free conversation.");
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Corrupt local chat/settings must never stop the Studio loading.
    }
    return () => {
  ...
  const startListening = () => {
    ...
    if (!recognitionRef.current) {
      ...
      recognition.onerror = (event) => {
        ...
        if (event.error === "not-allowed" || event.error === "service-not-allowed") {
          setStatus("Microphone permission is needed for Live Voice. Allow the mic in Chrome, then try again.");
    ...
    try {
      ...
      // Calling start while a recognition session is already active is harmless here.
    }
  };
  ...
  const send = async (providedText?: string, fromVoice = false) => {
    ...
    try {
      ...
      const reply = typeof raw === "string" ? raw : raw?.at(-1)?.content;
      const clean = String(
      ...
      setMessages([
        ...next,
        {
          role: "assistant",
          content:
            "The local brain couldn't start on this device. That's a device/runtime limitation, not a paid-service wall. Try the free WebGPU runner from the engine deck.",
        },
      ]);
      busyRef.current = false;
      setBusy(false);
  ...
  return (
    <Panel eyebrow="Buddy — Live Voice" title="Talk to Buddy" icon={<Sparkles className="size-5" />} defaultOpen>
      ...
      <div className="flex gap-2">
        ...
        />
        <StudioButton onClick={() => void send()} disabled={busy || !input.trim()} aria-label="Send message">
