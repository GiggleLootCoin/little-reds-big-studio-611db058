...
const VERSION = "Studio Version 5.0 — Free/Open Edition";
type TabId = "home" | "write" | "mix" | "video" | "community";
const TABS: { id: TabId; label: string; icon: typeof Compass }[] = [
  { id: "home", label: "Home", icon: Compass }, { id: "write", label: "Create", icon: Mic2 },
...
function Studio() {
  ...
  return <>
    <AnimatedBackground />
    {loading && <div className="fixed inset-0 z-50 flex flex-col items-center justify-center gap-5 bg-background/95 backdrop-blur-xl"><img src={logo.url} alt="Little Red's Big Studio" className="w-56 max-w-[72vw] animate-moon" onError={(event) => { event.currentTarget.src = "/favicon.ico"; }} /><div className="h-1 w-44 overflow-hidden rounded-full bg-secondary"><div className="gloss-sheen crimson-gloss h-full w-full" /></div><p className="font-display text-[0.65rem] font-semibold tracking-[0.28em] text-primary">{VERSION}</p></div>}
    <header className="sticky top-0 z-30 border-b border-border/50 bg-background/75 backdrop-blur-2xl"><div className="mx-auto flex w-full max-w-4xl items-center justify-center px-4 py-3.5"><a href="./" aria-label="Little Red's Big Studio home" className="group flex items-center justify-center rounded-2xl px-3 py-1.5 transition-transform hover:scale-[1.02] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"><img src={logo.url} alt="Little Red's Big Studio logo" className="h-12 w-auto max-w-[82vw] object-contain drop-shadow-[0_0_18px_hsl(var(--primary)/0.22)] sm:h-14" onError={(event) => { event.currentTarget.src = "/favicon.ico"; }} /></a></div><nav aria-label="Studio sections" className="mx-auto hidden w-full max-w-4xl gap-1 px-4 pb-2 sm:flex">{TABS.map((t) => <button key={t.id} type="button" aria-current={tab === t.id ? "page" : undefined} onClick={() => go(t.id)} className={cn("flex-1 rounded-xl px-3 py-2.5 font-display text-xs font-semibold tracking-wide transition-all", tab === t.id ? "crimson-gloss text-primary-foreground" : "text-muted-foreground hover:bg-secondary/50 hover:text-foreground")}>{t.label}</button>)}</nav></header>
