...
export const BUDDY_ENGINES: BuddyEngine[] = [
  ...
  { id: "kokoro", name: "Kokoro 82M", local: true, cloning: false, multilingual: true, minimumMemoryGb: 2 },
  { id: "chatterbox-onnx", name: "Chatterbox Multilingual ONNX", local: true, cloning: true, multilingual: true, minimumMemoryGb: 4 },
  { id: "qwen3-tts", name: "Qwen3-TTS 0.6B", local: true, cloning: true, multilingual: true, minimumMemoryGb: 6 },
  { id: "local-model", name: "Installed Buddy model", local: true, cloning: true, multilingual: true },
];

export function detectBuddyHardware() {
  if (typeof navigator === "undefined") return { webGpu: false, wasm: false, memoryGb: undefined as number | undefined };
...
export function chooseBuddyEngine(preferClone = false): BuddyEngine {
  ...
  if (preferClone && (hardware.memoryGb ?? 8) >= 6 && hardware.webGpu) return BUDDY_ENGINES.find((e) => e.id === "qwen3-tts")!;
  if (preferClone && (hardware.memoryGb ?? 8) >= 4) return BUDDY_ENGINES.find((e) => e.id === "chatterbox-onnx")!;
  if (hardware.webGpu || hardware.wasm) return BUDDY_ENGINES.find((e) => e.id === "kokoro")!;
  return BUDDY_ENGINES.find((e) => e.id === "browser")!;
}

export function speakBuddyLocally(text: string, voiceId = "browser-en-us", preferClone = false): boolean {
