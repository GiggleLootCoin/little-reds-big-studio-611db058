    ...
    <section className="rounded-2xl border border-primary/30 bg-background/55 p-4 shadow-[0_0_35px_hsl(var(--primary)/0.08)] backdrop-blur-md sm:p-5">
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
        {featured.map(({ id, icon: Icon, label, note }) => {
          ...
          return (
            <a key={id} href={runner.url} target="_blank" rel="noreferrer" className="group rounded-xl border border-border/70 bg-background/55 p-3 transition-all hover:-translate-y-0.5 hover:border-primary/50 hover:bg-primary/5">
