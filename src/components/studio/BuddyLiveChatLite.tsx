...
export function BuddyLiveChatLite() {
  ...
  const [microphoneId, setMicrophoneId] = useState(
    () => localStorage.getItem("lrbgs-buddy-mic") || "default",
  );
  ...
  const speak = async (text: string) => {
    ...
    try {
      ...
      const result = cloneMode
        ? await withTimeout(
            ...
            TTS_TIMEOUT_MS,
            "Buddy's natural voice timed out.",
          )
        : await withTimeout(
            runGradio(
              ...
              "",
              {
                ...
                instruct:
                  "Warm, natural human conversation. Vary rhythm, emphasis and pauses. Avoid announcer-like delivery, monotone cadence, and robotic phrasing. Sound like a thoughtful friend talking directly to one person.",
                model_size: "1.7B",
              },
              setStatus,
            ),
      ...
      audioRef.current.onended = () => {
        speakingRef.current = false;
        if (liveRef.current) window.setTimeout(() => void startListening(), 120);
      };
      ...
      console.warn("Natural Buddy voice unavailable", error);
    }
    ...
    setStatus(
      "Buddy is using the best voice available on this device while the natural voice route recovers…",
    );
  ...
  const send = async (forced?: string, voice = false) => {
    ...
    try {
      ...
      if (shouldResearch(text)) {
        setResearching(true);
        setStatus("Buddy is checking fresh information…");
        try {
      ...
      if (memories)
        context.push({
          role: "assistant",
          content: `Long-term memory that may matter:\n${memories}`,
        });
      if (research)
        context.push({ role: "assistant", content: `Fresh research notes:\n${research}` });
      context.push(...next);
      let reply = "";
      ...
      try {
        const result = await withTimeout(
          hordeText(prompt),
          CHAT_TIMEOUT_MS,
          "Buddy's community AI brain timed out.",
        );
        ...
        const result = await withTimeout(
          ...
          CHAT_TIMEOUT_MS,
          "Buddy's local AI brain timed out.",
        );
        reply = extract(result);
      }
      reply = reply.replace(/<think>[\s\S]*?<\/think>/gi, "").trim();
      if (!reply) throw new Error("Buddy's AI returned no usable response.");
      setMessages([...next, { role: "assistant", content: reply }]);
      void rememberConversation(text, reply).catch(() => undefined);
      setStatus("Buddy is ready.");
      if (voice || liveRef.current) void speak(reply);
    } catch (error) {
      const message = error instanceof Error ? error.message : "Buddy's AI could not respond.";
      setStatus(`Buddy could not reach a real AI brain: ${message}`);
  ...
  const transcribe = async (blob: Blob) => {
    ...
    try {
      ...
      if (!text)
        text = extract(
          await withTimeout(
            runGradio(FREE_SPACE_IDS.speechToText, "", { audio: blob }, setStatus),
            STT_TIMEOUT_MS,
            "Free speech recognition timed out.",
          ),
        );
      if (!text) throw new Error("No speech transcript was returned.");
      await send(text, true);
    } catch {
      if (liveRef.current) {
        setStatus("I didn't catch that. Listening again…");
        setTimeout(() => void startListening(), 180);
      } else setStatus("Speech recognition returned no transcript.");
    }
  };
  ...
  const startRecorderFallback = async () => {
    ...
    recorder.onstop = () => {
      ...
      setListening(false);
      stopMonitor();
      if (chunks.length && liveRef.current)
  ...
  const changeMicrophone = (id: string) => {
    setMicrophoneId(id);
    localStorage.setItem("lrbgs-buddy-mic", id);
    if (liveRef.current) {
      stopAll();
      setLive(false);
      setStatus("Microphone changed. Start Live Conversation again to use it.");
    }
  };
  ...
  return (
    ...
    >
      <div
        ...
        data-speaking={speakingRef.current}
      >
        ...
        <div>
          ...
          <span>{researching ? "Checking fresh information…" : status}</span>
        </div>
      </div>
      <div className="mt-3 flex flex-wrap items-center gap-2">
        ...
        <label className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/[.04] px-3 py-2 text-xs text-white/70">
          <Mic className="size-3.5" />
          <span>Microphone</span>
          <select
            value={microphoneId}
            onChange={(event) => changeMicrophone(event.target.value)}
            className="max-w-[190px] bg-transparent text-white outline-none"
      ...
      >
        {messages.length === 0 ? (
          <p className="text-sm text-white/45">
            Say hello or type a message. Buddy uses a real AI brain and keeps the conversation
            inside the Studio.
          </p>
        ) : (
          messages.map((message, index) => (
            <div
              ...
              className={`rounded-xl p-3 text-sm ${message.role === "user" ? "ml-8 bg-red-500/10 text-white/85" : "mr-8 bg-white/[.04] text-white/75"}`}
            >
      ...
      <p className="mt-2 text-[11px] text-white/30">
        Hands-free mode uses the microphone selected above. Headset microphones remain available as
