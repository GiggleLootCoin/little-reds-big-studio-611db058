  ...
  return (
    <div
      className={cn(
        ...
        compact ? "size-28" : "size-52 sm:size-64",
        className,
      )}
      ...
      <div className="buddy-character relative h-full w-full overflow-hidden rounded-full border border-white/15 bg-[radial-gradient(circle_at_50%_35%,#351019,#090509_70%)] shadow-[0_28px_90px_rgba(0,0,0,.7)]">
        <div className="buddy-head-motion absolute inset-[-3%]">
          <img
            src={buddyReference}
            alt="Buddy"
            className="buddy-character-image h-full w-full scale-[1.09] object-cover object-center"
          />
        ...
        <div
          className="absolute inset-x-[29%] bottom-[8%] flex h-[10%] items-end justify-center gap-1 opacity-85"
          aria-hidden
        >
          {[2, 5, 3, 7, 4, 6, 2].map((h, i) => (
            <span
              key={i}
              className="buddy-bar w-[7%] rounded-full bg-red-300"
              style={{ height: `${h * 10}%`, animationDelay: `${i * 70}ms` }}
            />
          ))}
        </div>
      </div>
      <span className="absolute -bottom-1 left-1/2 -translate-x-1/2 rounded-full border border-red-300/25 bg-black/75 px-3 py-1 text-[9px] font-black uppercase tracking-[.18em] text-red-200 shadow-lg backdrop-blur-md">
        {LABELS[status]}
      </span>
    </div>
  );
