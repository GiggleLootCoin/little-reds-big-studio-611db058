...
export function BuddyLiveChatLite() {
  ...
  const [microphoneId, setMicrophoneId] = useState(() => localStorage.getItem("lrbgs-buddy-mic") || "default");
  ...
  const speak = async (text: string) => {
    ...
    try {
      ...
      const result = cloneMode
        ...
        : await withTimeout(runGradio(FREE_SPACE_IDS.voicePreset, "", { text, language: "English", speaker: localStorage.getItem("lrbgs-buddy-voice-preset") || "Ryan", instruct: "Warm, natural human conversation. Vary rhythm, emphasis and pauses. Avoid announcer-like delivery, monotone cadence, and robotic phrasing. Sound like a thoughtful friend talking directly to one person.", model_size: "1.7B" }, setStatus), TTS_TIMEOUT_MS, "Buddy's natural voice timed out.");
      ...
      audioRef.current.onended = () => { speakingRef.current = false; if (liveRef.current) window.setTimeout(() => void startListening(), 120); };
      audioRef.current.onerror = () => { speakingRef.current = false; if (liveRef.current) window.setTimeout(() => void startListening(), 120); };
      await audioRef.current.play(); return;
    } catch (error) { console.warn("Natural Buddy voice unavailable", error); }
    if (!window.speechSynthesis) { speakingRef.current = false; if (liveRef.current) window.setTimeout(() => void startListening(), 120); return; }
    setStatus("Buddy is using the best voice available on this device while the natural voice route recovers…"); window.speechSynthesis.cancel(); const utterance = new SpeechSynthesisUtterance(text); utterance.rate = 0.94; utterance.pitch = 1.02;
    utterance.onend = () => { speakingRef.current = false; if (liveRef.current) window.setTimeout(() => void startListening(), 120); }; utterance.onerror = utterance.onend; window.speechSynthesis.speak(utterance);
  };

  const send = async (forced?: string, voice = false) => {
    const text = (forced ?? input).trim(); if (!text || busyRef.current) return;
    stopRecognition(); stopMonitor(); const next = [...messages, { role: "user" as const, content: text }].slice(-16); setMessages(next); setInput(""); busyRef.current = true; setBusy(true); setStatus("Buddy is thinking…");
    try {
      ...
      let reply = "";
      const prompt = `You are Buddy, the real AI creative partner inside Little Red's Big Studio. Speak like a warm, sharp, genuinely human friend who knows the user and is helping them create. Be natural, concise, expressive and conversational. Use contractions. Vary sentence length. Do not sound like a customer-service bot, do not use canned greetings, do not repeat the user's question, and do not mention providers, models, APIs, quotas or implementation details. If the user is speaking casually, respond casually. If they ask for help, take action-oriented initiative. Conversation:\n${context.map((m) => `${m.role}: ${m.content}`).join("\n")}`;
      try { const result = await withTimeout(hordeText(prompt), CHAT_TIMEOUT_MS, "Buddy's community AI brain timed out."); reply = extract(result); } catch {
      ...
      if (liveRef.current) setTimeout(() => void startListening(), 180);
    } finally { busyRef.current = false; setBusy(false); setResearching(false); }
  };

  const transcribe = async (blob: Blob) => {
    setStatus("Buddy is understanding you…"); try { let text = ""; try { text = extract(await withTimeout(runLocalSpeechToText(blob), STT_TIMEOUT_MS, "Local speech recognition timed out.")); } catch { /* free fallback */ } if (!text) text = extract(await withTimeout(runGradio(FREE_SPACE_IDS.speechToText, "", { audio: blob }, setStatus), STT_TIMEOUT_MS, "Free speech recognition timed out.")); if (!text) throw new Error("No speech transcript was returned."); await send(text, true); } catch { if (liveRef.current) { setStatus("I didn't catch that. Listening again…"); setTimeout(() => void startListening(), 180); } else setStatus("Speech recognition returned no transcript."); }
  };
  ...
  const startRecorderFallback = async () => {
    ...
    if (!streamRef.current) {
      const constraints: MediaStreamConstraints = { audio: microphoneId && microphoneId !== "default" ? { deviceId: { exact: microphoneId }, echoCancellation: true, noiseSuppression: true, autoGainControl: true } : { echoCancellation: true, noiseSuppression: true, autoGainControl: true } };
      streamRef.current = await navigator.mediaDevices.getUserMedia(constraints); await refreshMicrophones();
    }
  ...
  const startListening = async () => {
    ...
    // Use the selected MediaRecorder path for hands-free mode so the chosen microphone is honored.
  ...
  const toggle = () => { if (liveRef.current) { stopAll(); setLive(false); setStatus("Buddy is ready."); return; } liveRef.current = true; setLive(true); setStatus("Starting hands-free Live Conversation…"); void (async () => { try { if (!navigator.mediaDevices?.getUserMedia) throw new Error("Microphone capture unavailable"); if (!streamRef.current) { const constraints: MediaStreamConstraints = { audio: microphoneId && microphoneId !== "default" ? { deviceId: { exact: microphoneId }, echoCancellation: true, noiseSuppression: true, autoGainControl: true } : { echoCancellation: true, noiseSuppression: true, autoGainControl: true } }; streamRef.current = await navigator.mediaDevices.getUserMedia(constraints); await refreshMicrophones(); } await startListening(); } catch { liveRef.current = false; setLive(false); stopAll(); setStatus("Microphone access is required for Live Conversation."); } })(); };
  const changeMicrophone = (id: string) => { setMicrophoneId(id); localStorage.setItem("lrbgs-buddy-mic", id); if (liveRef.current) { stopAll(); setLive(false); setStatus("Microphone changed. Start Live Conversation again to use it."); } };

  useEffect(() => { void refreshMicrophones(); const handler = () => void refreshMicrophones(); navigator.mediaDevices?.addEventListener?.("devicechange", handler); return () => navigator.mediaDevices?.removeEventListener?.("devicechange", handler); }, []);
  ...
  return <Panel eyebrow="BUDDY • LIVE" title="Talk to Buddy" icon={<Sparkles className="size-5" />} defaultOpen>
    ...
    <div className="mt-3 flex flex-wrap items-center gap-2">
      ...
      <button type="button" onClick={() => { setMuted((current) => !current); if (!muted) { audioRef.current?.pause(); window.speechSynthesis?.cancel(); } }} className="rounded-xl border border-white/10 bg-white/[.04] px-3 py-2 text-xs font-semibold text-white/75">{muted ? <VolumeX className="mr-2 inline size-4" /> : <Volume2 className="mr-2 inline size-4" />}{muted ? "Muted" : "Sound On"}</button>
      <label className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/[.04] px-3 py-2 text-xs text-white/70"><Mic className="size-3.5" /><span>Microphone</span><select value={microphoneId} onChange={(event) => changeMicrophone(event.target.value)} className="max-w-[190px] bg-transparent text-white outline-none" aria-label="Choose microphone"><option value="default">System default microphone</option>{microphones.filter((device) => device.deviceId !== "default").map((device) => <option key={device.deviceId} value={device.deviceId}>{device.label || "Microphone"}</option>)}</select></label>
    </div>
    ...
    <p className="mt-2 text-[11px] text-white/30">Hands-free mode uses the microphone selected above. Headset microphones remain available as an additional option.</p>
  </Panel>;
}
