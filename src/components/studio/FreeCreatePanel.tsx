...
export function FreeCreatePanel() {
    ...
    try {
      const result = await withTimeout(
        runGradio(
          ...
          "/infer_music",
          {
            description: brief.trim() || "Create a polished original song about feeling loved.",
            lyrics: lyrics.trim(),
            lrc: lyrics.trim(),
            seed: -1,
            randomize_seed: true,
  ...
  const generateVideo = async () => {
    ...
    if (!videoImage) {
      setStatus("Add an image for your video first.");
      return;
    }
    if (!audio) {
      setStatus("Create a song or add audio first.");
      return;
    }
    setBusy("video");
    setVideoUrl(null);
    setStatus("Buddy is creating your video…");
    try {
      const result = await withTimeout(
        runGradio(
          FREE_SPACE_IDS.video,
          "/predict",
          {
            image: freeFile(videoImage),
            input_image: freeFile(videoImage),
            audio: typeof audio === "string" ? audio : freeFile(audio),
            prompt: brief.trim() || "Cinematic natural motion synchronized to the music, smooth camera movement.",
            duration_seconds: 5,
          },
          setStatus,
        ),
      ...
      setStatus(`Video could not be completed: ${messageOf(e)}`);
    } finally {
      setBusy(null);
    }
  };
  const cloneVoice = async () => {
    ...
    setBusy("clone");
    setCloneUrl(null);
    setStatus("Buddy is creating the voice result…");
    try {
      const result = await withTimeout(
        runGradio(
          ...
          {
            ref_audio: freeFile(referenceVoice),
            ref_text: "",
      ...
      setCloneUrl(url);
      setStatus("Your voice result is ready.");
    } catch (e) {
      setStatus(`Voice creation could not be completed: ${messageOf(e)}`);
    } finally {
      setBusy(null);
    }
  };
  const swapVoice = async () => {
    if (!swapSource || !referenceVoice) {
      setStatus("Choose both voice files first.");
      return;
    }
    setBusy("swap");
    setSwapUrl(null);
    setStatus("Buddy is transforming the voice…");
    try {
      ...
      setSwapUrl(url);
      setStatus("Your voice-swap result is ready.");
    } catch (e) {
      setStatus(`Voice swap could not be completed: ${messageOf(e)}`);
    } finally {
      setBusy(null);
    }
  };
    ...
    >
      ...
      <section className="rounded-2xl border border-border/70 bg-background/40 p-4">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-2"><FileText className="size-4 text-primary" /><h3 className="font-display font-semibold">Lyrics</h3></div>
          <StudioButton variant="ghost" onClick={() => void generateLyrics()} disabled={disabled}><WandSparkles className="size-4" />{busy === "lyrics" ? "Writing…" : "Generate lyrics"}</StudioButton>
        </div>
      ...
      <section className="rounded-2xl border border-border/70 bg-background/40 p-4">
        <div className="flex items-center gap-2"><Music2 className="size-4 text-primary" /><h3 className="font-display font-semibold">Music</h3></div>
        <p className="my-3 text-xs text-muted-foreground">Buddy chooses the appropriate track length automatically.</p>
        <StudioButton className="w-full" onClick={() => void generateSong()} disabled={disabled}>{busy === "song" ? <LoaderCircle className="size-4 animate-spin" /> : <Music2 className="size-4" />}{busy === "song" ? "Creating…" : "Generate song"}</StudioButton>
      </section>
      {audioUrl && <MediaResult kind="audio" url={audioUrl} />}
      <section className="grid gap-3 sm:grid-cols-2">
        <ActionCard title="Artwork" icon={<ImageIcon className="size-5" />} busy={busy === "image"} disabled={disabled} onClick={() => void generateImage()} />
        <ActionCard title="Video" icon={<Film className="size-5" />} busy={busy === "video"} disabled={disabled} onClick={() => void generateVideo()} />
      </section>
      ...
      <section className="rounded-2xl border border-border/70 bg-background/40 p-4">
        <div className="flex items-center gap-2"><Mic2 className="size-4 text-primary" /><h3 className="font-display font-semibold">Voice</h3></div>
        <div className="mt-3 grid gap-3 sm:grid-cols-2"><FilePick label="Reference voice" accept="audio/*" onFile={setReferenceVoice} /><FilePick label="Source voice for swap" accept="audio/*" onFile={setSwapSource} /></div>
        <div className="mt-3 grid grid-cols-2 gap-2">
          <StudioButton variant="ghost" onClick={() => void cloneVoice()} disabled={disabled}>{busy === "clone" ? <LoaderCircle className="size-4 animate-spin" /> : <Mic2 className="size-4" />}{busy === "clone" ? "Creating…" : "Clone voice"}</StudioButton>
          <StudioButton variant="ghost" onClick={() => void swapVoice()} disabled={disabled}>{busy === "swap" ? <LoaderCircle className="size-4 animate-spin" /> : <Mic2 className="size-4" />}{busy === "swap" ? "Creating…" : "Swap voice"}</StudioButton>
        </div>
      </section>
      {cloneUrl && <MediaResult kind="audio" url={cloneUrl} />}
      {swapUrl && <MediaResult kind="audio" url={swapUrl} />}
      <div className="flex items-center justify-between gap-3 rounded-xl border border-border/60 bg-background/30 p-3"><p className="text-xs text-muted-foreground" aria-live="polite">{status}</p><StudioButton variant="ghost" onClick={save}><Save className="size-4" />Save</StudioButton></div>
      <Note>Buddy handles the creation work behind the scenes and reports only the real result.</Note>
    </Panel>
  );
}
function ActionCard({ title, icon, busy, disabled, onClick }: { title: string; icon: React.ReactNode; busy: boolean; disabled: boolean; onClick: () => void }) {
  return <button type="button" onClick={onClick} disabled={disabled} className="group rounded-2xl border border-border/70 bg-background/40 p-5 text-left transition hover:-translate-y-0.5 hover:border-primary/50 hover:bg-primary/5 disabled:opacity-60"><span className="flex items-center justify-between"><span className="flex items-center gap-2 font-display font-semibold">{busy ? <LoaderCircle className="size-5 animate-spin text-primary" /> : icon}{busy ? "Creating…" : `Generate ${title}`}</span><span className="text-primary">→</span></span><span className="mt-2 block text-xs text-muted-foreground">Your finished {title.toLowerCase()} will appear here.</span></button>;
}
function FilePick({ label, accept, onFile }: { label: string; accept: string; onFile: (file: File | null) => void }) {
  return <label className="rounded-xl border border-dashed border-border bg-background/30 p-3 text-xs text-muted-foreground"><span className="font-medium text-foreground">{label}</span><input className="mt-2 block w-full text-xs" type="file" accept={accept} onChange={(e) => onFile(e.target.files?.[0] ?? null)} /></label>;
}
function MediaResult({ kind, url }: { kind: "audio" | "image" | "video"; url: string }) {
  return <div className="rounded-2xl border border-primary/25 bg-primary/5 p-3"><div className="mb-2 flex items-center justify-between"><span className="font-display text-xs font-bold uppercase tracking-wider text-primary">Ready</span><a href={url} target="_blank" rel="noreferrer" download className="inline-flex items-center gap-1 text-xs text-primary"><Download className="size-3.5" />Save</a></div>{kind === "audio" && <audio className="w-full" controls src={url} />}{kind === "image" && <img className="max-h-[28rem] w-full rounded-xl object-contain" src={url} alt="Your generated artwork" />}{kind === "video" && <video className="w-full rounded-xl" controls playsInline src={url} />}</div>;
}
