...
const runnerHint = (capability: string) => {
  const runner = FREE_RUNNERS.find((item) => item.capabilities.includes(capability)) || FREE_RUNNERS[0];
  return `\n\n### Free execution\nThis Studio does not call a paid AI API. Open **${runner.name}** and paste the prepared prompt: ${runner.url}`;
};
const prepared = (title: string, prompt: string, capability: string) => `## ${title}\n\n${prompt}\n\n_${HOUSE_STYLE}_` + runnerHint(capability);
...
export const critiqueSong = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => ({ critique: prepared("Free AI Song Coach Job", `Critique this song idea. Honesty ${data.honesty}/100; depth ${data.depth}/100.\nTitle: ${data.title}\nGenre: ${data.genre}\nNotes: ${data.notes}\nLyrics:\n${data.lyrics}\nReturn: verdict/10, strengths, weaknesses, specific lyric/arrangement alternatives, mix targets, next 3 moves.`, "text") }));
...
export const writeLyrics = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => ({ lyrics: prepared("Free Lyrics Job", `Write a full song about ${data.theme}. Genre ${data.genre}; mood ${data.mood}; rhyme density ${data.rhyme}/100; explicit=${data.explicit}. Include Intro, Verse 1, Pre-Chorus, Chorus, Verse 2, Bridge, Final Chorus and Outro, followed by performance notes.`, "text") }));
...
export const studioChat = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => ({ reply: prepared("Free Studio Chat Job", `Act as one practical creative assistant for Little Red's Big Studio. Focus areas: ${data.focus.join(", ") || "songwriting, production, vocals, artwork, video and YouTube"}. Give one decisive answer to:\n${data.messages.map((message) => `${message.role}: ${message.content}`).join("\n")}`, "text") }));
...
export const buildStoryboard = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => ({ storyboard: prepared("Free Storyboard Job", `Create exactly ${data.scenes} shot-by-shot music-video scenes for ${data.title}. ${data.bpm} BPM, ${data.durationSec}s. Direction: ${data.direction}. For each scene provide time/bars, shot, action, lighting/palette and a dense video prompt.`, "video") }));
...
export const generateSeo = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => ({ seo: prepared("Free YouTube SEO Job", `For ${data.title} by ${data.artist}, genre ${data.genre}, vibe ${data.vibe}: return 5 titles under 70 chars, a full description, 20 tags and 8 hashtags.`, "text") }));
...
export const generateArtwork = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => ({ prompt: `Create ${data.kind}: ${data.prompt}. Use Buddy reference when supplied: ${data.reference || "none"}.`, runner: FREE_RUNNERS.find((item) => item.capabilities.includes("image"))?.name || "Z Image Turbo" }));
