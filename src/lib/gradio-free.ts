...
function adaptInputs(
  ...
  if (
    logicalId === "voiceClone" &&
    space === "Qwen/Qwen3-TTS" &&
    endpoint === "/generate_voice_clone"
  ) {
    const values = Array.isArray(inputs) ? inputs : [];
    return [
      values[0],
      String(values[1] ?? ""),
      String(values[2] ?? "Hello from Buddy."),
      String(values[3] ?? "English"),
      Boolean(values[4] ?? true),
      String(values[5] ?? "1.7B"),
    ];
  }
  if (
    logicalId === "music" &&
