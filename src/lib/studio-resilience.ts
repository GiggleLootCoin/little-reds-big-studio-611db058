...
export function saveProjectSnapshot(snapshot: ProjectSnapshot): void {
  ...
  try {
    ...
    // Storage is an enhancement, never a reason to break creation.
  }
}
...
export function removeProjectSnapshot(id: string): void {
  if (typeof window === "undefined") return;
  try {
    localStorage.setItem(SNAPSHOT_KEY, JSON.stringify(readProjectSnapshots().filter((item) => item.id !== id)));
  } catch {
    // Best-effort cleanup.
  }
}
