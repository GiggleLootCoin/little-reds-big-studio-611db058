...
async function swapAndMix(sourceVocals: unknown, referenceVoice: Blob | File, instrumental: unknown, status: (s: string) => void) {
  ...
  const converted = await runGradioAll(FREE_SPACE_IDS.voiceSwap, "", {
    source_audio: vocalUrl, source_audio_path: vocalUrl, target_audio: referenceVoice, target_audio_path: referenceVoice,
    diffusion_steps: 30, length_adjust: 1, inference_cfg_rate: 0.7, f0_condition: true, auto_f0_adjust: true, pitch_shift: 0,
  }, status);
...
export function VocalStudioPanel() {
  ...
  const separate = async (input: File | string) => runGradioAll(FREE_SPACE_IDS.vocalSeparation, "", { audio: typeof input === "string" ? input : input }, setStatus);
  ...
  const swapSong = async () => {
    ...
    try {
      ...
      const separated = await separate(song);
      ...
      setResult(URL.createObjectURL(mixed)); setStatus("Finished. This is your voice on the original performance.");
  ...
  const makeSong = async () => {
    ...
    try {
      ...
      const generated = await runGradio(FREE_SPACE_IDS.music, "", {
        lrc: lyrics.trim() || "[verse]\nA new story starts tonight\n[chorus]\nWe are alive and moving forward",
        text_prompt: brief.trim() || "polished contemporary song with expressive lead vocal and full backing",
        description: brief.trim(), lyrics: lyrics.trim(), audio_duration: 90, duration: 90, seed: -1, randomize_seed: true,
      }, setStatus);
      sourceSong = artifactFor("music", "ACE-Step/Ace-Step-v1.5", generated) || "";
      if (!sourceSong) throw new Error("The music engine did not return a real song artifact.");
      let backingUrl: string | null = instrumental ? URL.createObjectURL(instrumental) : null;
      let vocalSource: unknown = sourceSong;
      if (!backingUrl) {
        setStatus("Buddy is separating the generated singer from the backing…");
        const separated = await separate(sourceSong);
        if (separated.length < 2) throw new Error("Buddy could not separate the generated vocal performance.");
        vocalSource = separated[0]; backingUrl = artifactFor("vocalSeparation", "nakas/demucs_playground", separated[1]);
      }
      if (!backingUrl) throw new Error("No backing track was available for the final mix.");
      const mixed = await swapAndMix(vocalSource, voice, backingUrl, setStatus);
      setResult(URL.createObjectURL(mixed)); setStatus("Finished. Your voice is now the lead singer on the new song.");
