...
async function swapAndMix(sourceVocals: unknown, referenceVoice: Blob | File, instrumental: unknown, status: (s: string) => void) {
  ...
  const converted = await runGradioAll(FREE_SPACE_IDS.voiceSwap, "/convert_voice_v1_wrapper", {
...
export function VocalStudioPanel() {
  ...
  const recordVoice = async () => {
    ...
    if (!navigator.mediaDevices?.getUserMedia) { setStatus("Your browser does not provide microphone recording. Upload a sample instead."); return; }
    try {
  ...
  const swapSong = async () => {
    ...
    try {
      ...
      const separated = await runGradioAll(FREE_SPACE_IDS.vocalSeparation, "/predict", [freeFile(song)], setStatus);
      if (separated.length < 2) throw new Error("The separation engine did not return both vocal and instrumental tracks.");
      const mixed = await swapAndMix(separated[0], voice, separated[1], setStatus);
      setResult(URL.createObjectURL(mixed));
      setStatus("Finished. This is your voice singing the original performance over the original music.");
    } catch (error) {
  ...
  const makeSong = async () => {
    ...
    try {
      ...
      if (!backing) {
        ...
        const generated = await runGradio(FREE_SPACE_IDS.music, "/create", {
        ...
        sourceSongUrl = outputUrl(generated);
        if (!sourceSongUrl) throw new Error("The singing-performance engine did not return a source performance.");
      }
      setStatus("Buddy is separating the generated performance so only the singer changes…");
      const separated = await runGradioAll(FREE_SPACE_IDS.vocalSeparation, "/predict", [sourceSongUrl], setStatus);
      if (separated.length < 2) throw new Error("Buddy could not separate the generated vocal performance.");
  ...
  return (
    <Panel eyebrow="VOCALS • YOUR VOICE" title="Make the song yours" icon={<Music2 className="size-5" />} defaultOpen>
      <div className="rounded-2xl border border-primary/20 bg-primary/5 p-4">
        <div className="flex items-start gap-3">
          <span className="flex size-11 shrink-0 items-center justify-center rounded-2xl bg-primary/15 text-primary"><Mic className="size-5" /></span>
          <div>
            <h3 className="font-display font-black">One voice sample. Buddy does the technical work.</h3>
            <p className="mt-1 text-xs leading-5 text-muted-foreground">Record yourself or upload a sample you own. Buddy automatically handles separation, voice conversion, timing and the final mix.</p>
          </div>
        </div>
        <div className="mt-4 grid gap-2 sm:grid-cols-2">
          ...
          <label className="flex cursor-pointer items-center justify-center gap-2 rounded-xl border border-border bg-background/60 px-4 py-2.5 text-sm font-semibold hover:bg-background">
            ...
            <input type="file" accept="audio/*" className="sr-only" disabled={disabled} onChange={(e) => setVoice(e.target.files?.[0] ?? null)} />
          </label>
        </div>
      ...
      <section className="rounded-2xl border border-border/70 bg-background/40 p-4">
        <div className="flex items-start gap-3"><span className="flex size-10 items-center justify-center rounded-xl border border-primary/20 bg-primary/10 text-primary">01</span><div><h3 className="font-display font-black">Put your voice on any song</h3><p className="text-xs text-muted-foreground">Upload a song. Buddy removes the original singer, preserves the music, puts your voice onto the performance and returns one finished track.</p></div></div>
        <label className="mt-4 flex cursor-pointer items-center gap-3 rounded-xl border border-dashed border-border p-4 text-sm"><Upload className="size-4 text-primary" /><span className="min-w-0 flex-1 truncate">{song?.name ?? "Choose the song to transform"}</span><input type="file" accept="audio/*" className="sr-only" disabled={disabled} onChange={(e) => setSong(e.target.files?.[0] ?? null)} /></label>
        <StudioButton className="mt-3 w-full" onClick={() => void swapSong()} disabled={disabled || !voice || !song}>{busy === "swap" ? <LoaderCircle className="size-4 animate-spin" /> : <WandSparkles className="size-4" />}{busy === "swap" ? "Buddy is doing the whole swap…" : "Make me the singer"}</StudioButton>
      </section>

      <section className="rounded-2xl border border-border/70 bg-background/40 p-4">
        <div className="flex items-start gap-3"><span className="flex size-10 items-center justify-center rounded-xl border border-primary/20 bg-primary/10 text-primary">02</span><div><h3 className="font-display font-black">Create a new song with my voice</h3><p className="text-xs text-muted-foreground">Give Buddy lyrics or an idea. Add an instrumental if you already have one; otherwise Buddy creates the backing track too.</p></div></div>
        <textarea value={lyrics} onChange={(e) => setLyrics(e.target.value)} rows={5} placeholder="Paste or write lyrics here…" className="mt-4 w-full rounded-xl border border-border bg-background/70 p-3 text-sm" disabled={disabled} />
        <input value={brief} onChange={(e) => setBrief(e.target.value)} placeholder="Optional: describe genre, mood and feel…" className="mt-2 w-full rounded-xl border border-border bg-background/70 p-3 text-sm" disabled={disabled} />
        <label className="mt-3 flex cursor-pointer items-center gap-3 rounded-xl border border-dashed border-border p-4 text-sm"><Upload className="size-4 text-primary" /><span className="min-w-0 flex-1 truncate">{instrumental?.name ?? "Optional instrumental / backing track"}</span><input type="file" accept="audio/*" className="sr-only" disabled={disabled} onChange={(e) => setInstrumental(e.target.files?.[0] ?? null)} /></label>
        <StudioButton className="mt-3 w-full" onClick={() => void makeSong()} disabled={disabled || !voice || (!lyrics.trim() && !brief.trim())}>{busy === "make" ? <LoaderCircle className="size-4 animate-spin" /> : <Music2 className="size-4" />}{busy === "make" ? "Buddy is creating your song…" : "Create my song"}</StudioButton>
      </section>
      ...
      <Note>Buddy automatically chooses the best available free engine and keeps technical controls out of your way. Only use voice samples and songs you own or are authorized to transform.</Note>
    </Panel>
  );
}
