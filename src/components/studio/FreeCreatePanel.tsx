...
export function FreeCreatePanel() {
  ...
  const [videoImage, setVideoImage] = useState<File | null>(null); const [videoAudio, setVideoAudio] = useState<File | null>(null); const [referenceVoice, setReferenceVoice] = useState<File | null>(null); const [swapSource, setSwapSource] = useState<File | null>(null);
  useEffect(() => { setBrief(localStorage.getItem("lrbgs-song-brief") || ""); setLyrics(localStorage.getItem("lrbgs-lyrics") || ""); }, []);
  const generateLyrics = async () => { setBusy("lyrics"); setStatus("Buddy is writing your lyrics…"); try {
    const dynamicImport = new Function("url", "return import(url)") as (url: string) => Promise<any>;
    const lib = await withTimeout(dynamicImport("https://cdn.jsdelivr.net/npm/@huggingface/transformers@4.2.0"), 60000);
    const hasGpu = typeof navigator !== "undefined" && "gpu" in navigator;
    const model: any = await withTimeout(lib.pipeline("text-generation", "onnx-community/Qwen3-0.6B-ONNX", { dtype: hasGpu ? "q4f16" : "q4", device: hasGpu ? "webgpu" : "wasm" }), 120000);
    const result = await withTimeout(model([{ role: "system", content: "Write original singable lyrics. Do not quote existing lyrics or imitate living artists." }, { role: "user", content: `Write original lyrics for ${brief.trim() || "a song about feeling loved"}. Include Verse 1, Pre-Chorus, Chorus, Verse 2, Bridge and Final Chorus.` }], { max_new_tokens: 650, temperature: 0.9, do_sample: true, return_full_text: false }), 120000);
    const first = Array.isArray(result) ? result[0] : result; const generated = typeof first?.generated_text === "string" ? first.generated_text : first?.generated_text?.at?.(-1)?.content; if (!generated) throw new Error("No lyrics were returned.");
    const clean = generated.replace(/<think>[\s\S]*?<\/think>/gi, "").trim(); setLyrics(clean); localStorage.setItem("lrbgs-lyrics", clean); setStatus("Your lyrics are ready.");
  } catch (e) { setStatus(`Lyrics could not be completed: ${messageOf(e)}`); } finally { setBusy(null); } };
