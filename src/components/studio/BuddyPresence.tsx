...
const STATUS_LABELS: Record<BuddyStatus, string> = { idle: "Ready", listening: "Listening", thinking: "Thinking", working: "Working", success: "Sorted", error: "Needs another go" };
const STATUS_ICONS = { idle: Sparkles, listening: Ear, thinking: Brain, working: Loader2, success: CheckCircle2, error: AlertCircle } satisfies Record<BuddyStatus, typeof Sparkles>;
...
export function BuddyPresence({ className }: { className?: string }) {
  ...
  return (
    <aside className={cn("buddy-presence glass-panel relative overflow-hidden rounded-2xl p-3", className)} data-buddy-status={snapshot.status} aria-label={`Buddy: ${STATUS_LABELS[snapshot.status]}`}>
      <div className="flex items-center gap-3">
        ...
        <img src={logo.url} alt="Little Red's Big Studio" className="brand-mark-animated hidden h-8 w-auto shrink-0 rounded-md border border-white/10 bg-black/50 p-1 sm:block" />
      </div>
    </aside>
  );
}
