...
function fallbackReply(text: string) {
  ...
  if (q.includes("voice"))
    return "I can work with a voice you own or have permission to use. Tell me what you want the voice to do.";
...
function transcriptionText(value: unknown): string {
  ...
  if (Array.isArray(value))
    return transcriptionText(value.find((x) => typeof x === "string") ?? value[0]);
  if (!value || typeof value !== "object") return "";
  const record = value as Record<string, unknown>;
...
export function BuddyLiveChatLite() {
  ...
  const speak = (text: string) => {
    ...
    utterance.onend = () => {
      speakingRef.current = false;
      if (handsFreeRef.current) startListenCycle();
  ...
  const send = async (forced?: string, fromVoice = false) => {
    ...
    try {
      ...
      try {
        ...
        console.warn("Local Buddy brain unavailable; fallback used", error);
  ...
  const startListenCycle = async () => {
    if (!handsFreeRef.current || busyRef.current || speakingRef.current || recorderRef.current)
      return;
    try {
      ...
      recorder.onerror = () => {
        recorderRef.current = null;
        setListening(false);
        if (handsFreeRef.current) scheduleNextListen();
      };
      ...
      setStatus(
        "Microphone access is required for Live Conversation. Allow it in Chrome and try again.",
      );
    }
  };
