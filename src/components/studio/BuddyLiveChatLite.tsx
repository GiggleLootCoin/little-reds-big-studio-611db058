...
export function BuddyLiveChatLite() {
  ...
  const speakNaturally = async (text: string) => {
    if (muted) { speakingRef.current = false; resumeListening(); return; }
    ...
    try {
      ...
      audioRef.current.onended = () => { speakingRef.current = false; resumeListening(); };
      audioRef.current.onerror = () => { throw new Error("TTS audio could not be played."); };
      await audioRef.current.play();
      return;
    } catch (error) {
      console.warn("Natural Buddy TTS unavailable; using device voice", error);
    }
    if (!window.speechSynthesis) { speakingRef.current = false; resumeListening(); return; }
    window.speechSynthesis.cancel();
    const utterance = new SpeechSynthesisUtterance(text);
    utterance.rate = 0.98;
    utterance.pitch = 1;
    utterance.onend = () => { speakingRef.current = false; resumeListening(); };
    utterance.onerror = () => { speakingRef.current = false; resumeListening(); };
  ...
  const send = async (forced?: string, voice = false) => {
    ...
    try {
      ...
      const result = await brainRef.current([
        { role: "system", content: `You are Buddy inside Little Red's Big Studio. Address this user as ${name}. Never call all users Red. Be concise, warm, practical and honest.` },
        ...next,
  ...
  const schedule = () => {
    ...
    if (timerRef.current) window.clearTimeout(timerRef.current);
    timerRef.current = window.setTimeout(() => { timerRef.current = null; void start(); }, 250);
  };
  ...
  const toggle = () => {
    if (liveRef.current) { stop(); setLive(false); setStatus("Buddy is ready."); }
    else { liveRef.current = true; setLive(true); setStatus("Starting Live Conversation…"); void start(); }
  };
  ...
  return (
    <Panel eyebrow="BUDDY • LIVE" title={`Talk to Buddy, ${name}`} icon={<Sparkles className="size-5" />} defaultOpen>
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <button type="button" onClick={() => { setMuted(!muted); if (!muted) { audioRef.current?.pause(); window.speechSynthesis?.cancel(); } }} className="rounded-xl border border-border bg-background/60 px-3 py-2 text-xs font-semibold">
      ...
      <div className="flex gap-2">
        <input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") void send(); }} disabled={busy} placeholder={`Talk to Buddy, ${name}…`} className="min-w-0 flex-1 rounded-xl border border-border bg-background/60 px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-ring" />
