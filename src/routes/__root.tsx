...
import studioPolishCss from "../studio-polish.css?url";
...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
  head: () => ({
    meta: [
      { charSet: "utf-8" }, { name: "viewport", content: "width=device-width, initial-scale=1" }, { title: "Little Red's Big Studio" },
      { name: "description", content: "Free/open Android-first creative studio for music and creators." }, { name: "author", content: "LittleRedBigSmile" }, { name: "theme-color", content: "#0d0405" }, { property: "og:type", content: "website" }, { name: "twitter:card", content: "summary_large_image" },
    ],
    links: [{ rel: "stylesheet", href: appCss }, { rel: "stylesheet", href: studioPolishCss }, { rel: "icon", href: `${import.meta.env.BASE_URL}favicon.ico`, type: "image/x-icon" }],
  }),
...
function RootComponent() {
  ...
  return <QueryClientProvider client={queryClient}><StudioSafetyNet /><Outlet /></QueryClientProvider>;
}
