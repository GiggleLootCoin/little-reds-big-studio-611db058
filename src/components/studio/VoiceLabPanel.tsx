...
const VOICES = [
  ["Ryan", "English • Ryan"], ["Aiden", "English • Aiden"], ["Dylan", "English • Dylan"], ["Eric", "English • Eric"],
  ["Serena", "English • Serena"], ["Sohee", "Korean/English • Sohee"], ["Vivian", "English • Vivian"], ["Ono_anna", "Japanese • Ono Anna"], ["Uncle_fu", "Chinese • Uncle Fu"],
] as const;
...
export async function loadStoredBuddyVoice(): Promise<Blob | null> {
  ...
  return new Promise((resolve) => {
    ...
    request.onsuccess = () => {
      ...
      get.onsuccess = () => { db.close(); resolve(get.result instanceof Blob ? get.result : null); };
...
export function VoiceLabPanel() {
  ...
  const preview = async () => {
    setBusy(true); setAudioUrl(null); setStatus("Finding a live natural voice engine…");
    try {
      const result = await runGradio(FREE_SPACE_IDS.voicePreset, "", {
        text, language: "English", speaker: voice,
        instruct: "Natural, warm conversational delivery with realistic pauses, varied pacing and gentle breaths.", model_size: "1.7B",
      }, setStatus);
      const space = lastSuccessfulFreeSpace("voicePreset", "Qwen/Qwen3-TTS");
      const url = freeArtifactUrl(result, space);
      if (!url) throw new Error("The live voice engine returned no playable audio.");
  ...
  const clone = async () => {
    ...
    setBusy(true); setAudioUrl(null); setStatus("Creating a real voice-clone preview…");
    try {
      await storeReference(reference);
      localStorage.setItem("lrbgs-buddy-voice-language", "English");
      const result = await runGradio(FREE_SPACE_IDS.voiceClone, "", {
        ref_audio: reference, ref_text: "", target_text: text, language: "English", use_xvector_only: true, model_size: "1.7B",
      }, setStatus);
      const space = lastSuccessfulFreeSpace("voiceClone", "Qwen/Qwen3-TTS");
      const url = freeArtifactUrl(result, space);
      if (!url) throw new Error("The live cloning engine returned no playable audio.");
  ...
  return (
    <Panel eyebrow="VOICE LAB" title="Buddy's voice" icon={<Mic2 className="size-5" />} defaultOpen>
      ...
      <Note>Buddy keeps your reference voice in this browser's local storage. The selected free public engine receives the sample only for that generation request. Only use voice material you own or are authorized to submit.</Note>
    </Panel>
  );
}
