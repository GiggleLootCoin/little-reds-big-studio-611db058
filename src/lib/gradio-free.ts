...
const ROUTES: Record<string, RouteCandidate[]> = {
  ...
  voiceClone: [
    {
      ...
      endpoints: ["/generate_voice_clone", "/generate_custom_voice"],
      priority: 180,
    },
  ...
  voicePreset: [
    {
      space: "Qwen/Qwen3-TTS",
      endpoints: ["/generate_custom_voice", "/generate_speech"],
      priority: 180,
    },
  ...
  vocalSeparation: [
    {
      ...
      endpoints: ["/inference", "/separate", "/predict"],
      priority: 180,
    },
    {
...
async function resolveEndpoint(route: RouteCandidate, preferredEndpoint?: string) {
  ...
  const candidates = preferredEndpoint
    ? [preferredEndpoint, ...route.endpoints.filter((x) => x !== preferredEndpoint)]
    : route.endpoints;
...
function adaptInputs(
  ...
  if (logicalId === "speechToText") {
    const audio = !Array.isArray(inputs) ? inputs.audio : inputs[0];
    return endpoint === "/transcribe"
