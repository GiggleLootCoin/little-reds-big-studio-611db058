...
export const writeLyrics = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => ({
    lyrics: await realText(
      `Write a complete original song about ${data.theme}. Genre ${data.genre}; mood ${data.mood}; rhyme density ${data.rhyme}/100; explicit=${data.explicit}. Include Intro, Verse 1, Pre-Chorus, Chorus, Verse 2, Bridge, Final Chorus and Outro. Do not quote existing lyrics.`,
    ),
  }));
...
export const studioChat = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => ({
    reply: await realText(
      `Act as Buddy, one practical creative assistant for Little Red's Big Studio. Focus areas: ${data.focus.join(", ") || "songwriting, production, vocals, artwork, video and YouTube"}. Give one decisive answer. Conversation:\n${data.messages.map((message) => `${message.role}: ${message.content}`).join("\n")}`,
    ),
  }));
...
export const buildStoryboard = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => ({
    storyboard: await realText(
      `Create exactly ${data.scenes} shot-by-shot music-video scenes for ${data.title}. ${data.bpm} BPM, ${data.durationSec}s. Direction: ${data.direction}. For each scene provide timing/bars, shot, action, lighting/palette and a dense generation prompt.`,
    ),
  }));
...
export const generateSeo = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => ({
    seo: await realText(
      `For ${data.title} by ${data.artist}, genre ${data.genre}, vibe ${data.vibe}: return 5 titles under 70 characters, a complete YouTube description, 20 tags and 8 hashtags.`,
    ),
  }));
