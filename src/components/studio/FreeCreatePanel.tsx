...
export function FreeCreatePanel() {
  ...
  const generateSong = async () => {
    ...
    try {
      const prompt = `${brief.trim() || "Create an original song about feeling loved."}\nLyrics:\n${lyrics.trim() || "Create suitable original lyrics."}`;
      const result = await withTimeout(
        runGradio(FREE_SPACE_IDS.music, "/create", {
          description: prompt,
          seed: -1,
          community: false,
        }),
  ...
  const generateVideo = async () => {
    ...
    try {
      const result = await withTimeout(
        runGradio(FREE_SPACE_IDS.video, "/predict", [
          freeFile(videoImage),
          freeFile(audio),
          "480P",
        ]),
  ...
  const cloneVoice = async () => {
    ...
    try {
      const result = await withTimeout(
        runGradio(FREE_SPACE_IDS.voiceClone, "/generate_voice_clone", {
  ...
  const swapVoice = async () => {
    ...
    try {
      const outputs = await withTimeout(
        runGradioAll(FREE_SPACE_IDS.voiceSwap, "/convert_voice_v1_wrapper", {
    ...
    >
      <div className="rounded-2xl border border-primary/20 bg-primary/5 p-4">
        ...
        <textarea
          ...
          rows={4}
          placeholder="Tell Buddy what you want to create…"
          className="mt-3 w-full rounded-xl border border-border bg-background/70 p-3 text-sm outline-none focus:ring-2 focus:ring-ring"
        />
      </div>
      <section className="rounded-2xl border border-border/70 bg-background/40 p-4">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-display font-semibold">Lyrics</h3>
          </div>
      ...
      <section className="rounded-2xl border border-border/70 bg-background/40 p-4">
        ...
        <p className="my-3 text-xs text-muted-foreground">
          Buddy chooses the appropriate track length automatically.
        </p>
        <StudioButton className="w-full" onClick={() => void generateSong()} disabled={disabled}>
          ...
          {busy === "song" ? "Creating…" : "Generate song"}
        </StudioButton>
      </section>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <FilePick label="Video image" accept="image/*" onFile={setVideoImage} />
        <FilePick label="Video audio (optional)" accept="audio/*" onFile={setVideoAudio} />
      </div>
      ...
      <section className="rounded-2xl border border-border/70 bg-background/40 p-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-display font-semibold">Voice</h3>
        </div>
        <div className="mt-3 grid gap-3 sm:grid-cols-2">
          <FilePick label="Reference voice" accept="audio/*" onFile={setReferenceVoice} />
          <FilePick label="Source voice for swap" accept="audio/*" onFile={setSwapSource} />
        </div>
        <div className="mt-3 grid grid-cols-2 gap-2">
          <StudioButton variant="ghost" onClick={() => void cloneVoice()} disabled={disabled}>
            ...
            {busy === "clone" ? "Creating…" : "Clone voice"}
          </StudioButton>
      ...
      <Note>
        Buddy handles the creation work behind the scenes and reports only the real result.
      </Note>
    </Panel>
  );
}
function ActionCard({
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className="group rounded-2xl border border-border/70 bg-background/40 p-5 text-left transition hover:-translate-y-0.5 hover:border-primary/50 hover:bg-primary/5 disabled:opacity-60"
    >
      <span className="flex items-center justify-between">
        <span className="flex items-center gap-2 font-display font-semibold">
          {busy ? <LoaderCircle className="size-5 animate-spin text-primary" /> : icon}
          {busy ? "Creating…" : `Generate ${title}`}
        </span>
        <span className="text-primary">→</span>
      </span>
      <span className="mt-2 block text-xs text-muted-foreground">
        Your finished {title.toLowerCase()} will appear here.
      </span>
    </button>
  );
}
function FilePick({
  ...
  return (
    <label className="rounded-xl border border-dashed border-border bg-background/30 p-3 text-xs text-muted-foreground">
