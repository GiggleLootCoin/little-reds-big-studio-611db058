...
function artifactFor(logical: string, fallback: string, value: unknown) {
  return freeArtifactUrl(value, lastSuccessfulFreeSpace(logical, fallback));
}
async function swapAndMix(
  ...
  const converted = await runGradioAll(
    ...
    "",
    {
      source_audio: vocalUrl,
      source_audio_path: vocalUrl,
      target_audio: referenceVoice,
      target_audio_path: referenceVoice,
      diffusion_steps: 30,
      length_adjust: 1,
...
export function VocalStudioPanel() {
  ...
  const recordVoice = async () => {
    ...
    if (!navigator.mediaDevices?.getUserMedia || typeof MediaRecorder === "undefined") {
      setStatus("Microphone recording is unavailable here. Upload a sample instead.");
      return;
    }
    try {
      ...
      recorder.onstop = () => {
        ...
        setVoice(new Blob(chunksRef.current, { type: recorder.mimeType || "audio/webm" }));
        setRecording(false);
      ...
      recorder.start();
      setRecording(true);
      setStatus("Recording your reference voice… tap again when ready.");
    } catch {
      setStatus("Microphone access was not available. Upload your voice sample instead.");
    }
  };
  const separate = async (input: File | string) =>
    runGradioAll(FREE_SPACE_IDS.vocalSeparation, "", { audio: input }, setStatus);
  const swapSong = async () => {
    ...
    try {
      ...
      setStatus("Finished. This is your voice on the original performance.");
    } catch (error) {
    ...
    try {
      ...
      const generated = await runGradio(
        ...
        "",
        {
          ...
          text_prompt:
            brief.trim() ||
            "polished contemporary song with expressive lead vocal and full backing",
      ...
      if (!backingUrl) {
        ...
        vocalSource = separated[0];
        backingUrl = artifactFor("vocalSeparation", "nakas/demucs_playground", separated[1]);
      }
    ...
    >
      <div className="rounded-2xl border border-primary/20 bg-primary/5 p-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-xs leading-5 text-muted-foreground">
              Record yourself or upload a sample you own. Buddy handles separation, voice conversion
              and the final mix.
            </p>
          </div>
        </div>
        <div className="mt-4 grid gap-2 sm:grid-cols-2">
          ...
          <label className="flex cursor-pointer items-center justify-center gap-2 rounded-xl border border-border bg-background/60 px-4 py-2.5 text-sm font-semibold">
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Upload a song. Buddy removes the original singer, converts the performance and returns
              a finished track.
            </p>
          </div>
        </div>
      ...
      <section className="rounded-2xl border border-border/70 bg-background/40 p-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Give Buddy lyrics or an idea. Add an instrumental if you already have one.
            </p>
          </div>
        </div>
        <textarea
      ...
      <Note>
        Buddy automatically chooses live free public engines. Only use voice samples and songs you
