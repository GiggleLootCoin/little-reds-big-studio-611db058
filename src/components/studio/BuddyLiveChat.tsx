...
type GeneratorOutput = Array<{ generated_text?: GeneratedText }> | { generated_text?: GeneratedText };
type Generator = (messages: PromptMessage[], options: Record<string, unknown>) => Promise<GeneratorOutput>;
type TransformersModule = { pipeline: (task: string, model: string, options: Record<string, unknown>) => Promise<unknown> };
...
export function BuddyLiveChat() {
  ...
  return (
    <Panel eyebrow="Buddy — unlimited local chat" title="Talk to Buddy" icon={<Sparkles className="size-5" />} defaultOpen>
      <div className="flex items-center gap-2 rounded-xl border border-primary/20 bg-primary/5 p-3 text-xs text-muted-foreground"><Brain className="size-4 shrink-0 text-primary" /><span>{status}</span></div>
