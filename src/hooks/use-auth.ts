...
function readUser(): LocalUser | null {
  ...
  try {
    return JSON.parse(localStorage.getItem(KEY) || "null") as LocalUser | null;
  } catch {
    return null;
  }
}
...
export function useProfile(userId: string | undefined) {
  ...
  const load = async (id: string) => {
    setLoading(true);
    const raw = localStorage.getItem(`little-reds-profile:${id}`);
    const user = readUser();
    setProfile(
      raw
        ? JSON.parse(raw)
        : {
            id,
            handle:
              user?.user_metadata.display_name?.toLowerCase().replace(/\s+/g, "-") || "creator",
