...
export function connectFreeSpace(space: string) {
  ...
  if (!client) { client = loadGradio().then(({ Client }) => Client.connect(space)); clients.set(space, client); }
  return client;
}
...
export function outputUrl(value: unknown): string | null {
  if (typeof value === "string") {
    ...
    if (s.startsWith("{") || s.startsWith("[")) { try { return outputUrl(JSON.parse(s)); } catch (error) { console.warn("Could not parse media result", error); } }
    return null;
  }
...
export function firstOutput(result: unknown): unknown {
  if (typeof result === "string") { try { return firstOutput(JSON.parse(result)); } catch (error) { console.warn("Could not parse Gradio result", error); return result; } }
...
export async function runGradioAll(space: string, apiName: string, inputs: Record<string, unknown> | unknown[]) {
  ...
  if (latest && typeof latest === "object") { const data = (latest as { data?: unknown }).data; if (Array.isArray(data)) return data; }
