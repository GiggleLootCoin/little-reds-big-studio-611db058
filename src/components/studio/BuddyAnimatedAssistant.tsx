    ...
    >
      ...
      <div className="buddy-character relative h-full w-full overflow-hidden rounded-full border border-primary/25 bg-[radial-gradient(circle_at_50%_38%,oklch(0.22_0.08_22),oklch(0.07_0.02_18)_72%)] shadow-[0_24px_80px_oklch(0_0_0_/_0.65)]">
        <img
          src={buddyReference}
          alt="Buddy"
          className="buddy-character-image h-full w-full scale-[1.08] object-cover object-center"
        />
        ...
        <div
          className="absolute inset-x-[30%] bottom-[9%] flex h-[9%] items-end justify-center gap-1 opacity-80"
          aria-hidden
        >
          {[2, 5, 3, 7, 4, 6, 2].map((h, i) => (
            <span
              key={i}
              className="buddy-bar w-[7%] rounded-full bg-primary"
              style={{ height: `${h * 10}%`, animationDelay: `${i * 70}ms` }}
            />
          ))}
        </div>
      </div>
      <span className="absolute -bottom-1 left-1/2 -translate-x-1/2 rounded-full border border-primary/35 bg-black/80 px-3 py-1 text-[9px] font-bold uppercase tracking-[0.18em] text-primary shadow-lg backdrop-blur-md">
        {LABELS[status]}
      </span>
    </div>
  );
