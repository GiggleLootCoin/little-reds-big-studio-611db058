...
async function fetchText(url: string) {
  ...
  try {
    const response = await fetch(url, { signal: controller.signal, headers: { Accept: "text/plain,text/markdown,*/*" } });
...
export function shouldResearch(text: string) {
  return /\b(search|research|look up|find out|latest|today|current|recent|news|what's happening|what is happening|who is|how much|price|weather|trending|verify|fact[- ]?check|source|sources)\b/i.test(text);
}
...
export async function liveWebSearch(query: string): Promise<WebResult[]> {
  ...
  for (const target of targets) {
    ...
    } catch (error) { console.warn("Buddy web search route failed", error); }
  }
  return [];
}
...
export async function researchWeb(query: string) {
  ...
  if (!results.length) return "I couldn't reach a live search source right now, so I won't pretend I checked the web.";
  return [
    "Live web research:",
    ...results.slice(0, 6).map((result, index) => `${index + 1}. ${result.title}\n${result.source}\n${result.snippet}\n${result.url}`),
  ].join("\n\n");
}

export function sourceLabel(url: string) { try { return new URL(decode(url)).hostname.replace(/^www\./, ""); } catch { return "web"; } }
