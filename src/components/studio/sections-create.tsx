...
import { Clapperboard, Film, Scissors, SlidersHorizontal, UploadCloud } from "lucide-react";
...
export function QRangePanel() {
  ...
  return <Panel eyebrow="Mix core" title="Red's QRange" icon={<SlidersHorizontal className="size-5" />} defaultOpen>
...
export function UploadPanel() {
  ...
  return <Panel eyebrow="Local files" title="Audio, Voice & File Uploads" icon={<UploadCloud className="size-5" />}>
    <button type="button" onClick={() => inputRef.current?.click()} className="w-full rounded-2xl border-2 border-dashed border-border bg-background/40 p-7 text-center transition-colors hover:border-primary hover:bg-primary/5">
      <UploadCloud className="mx-auto mb-2 size-8 text-primary" /><p className="font-display text-sm">Choose audio or reference imagery</p><p className="mt-1 text-xs text-muted-foreground">The browser holds this session. No paid storage backend.</p>
...
export function LabPanel() {
  ...
  return <Panel eyebrow="Audio lab" title="Red'sLab Stem Studio" icon={<Scissors className="size-5" />}>
...
export function StoryboardPanel() {
  ...
  const build = () => {
    ...
    setResult(text); setStudio({ storyboard: text, direction: base });
  };
  return <Panel eyebrow="Video prep" title="Automated Storyboarding" icon={<Clapperboard className="size-5" />}>
...
export function VideoPanel() {
  ...
  const open = async () => { try { await navigator.clipboard.writeText(prompt); } catch { /* optional */ } window.open(wan.url, "_blank", "noopener,noreferrer"); };
  return <Panel eyebrow="Video" title="Free Video Generation" icon={<Film className="size-5" />}>
    <p className="text-sm text-muted-foreground">Your phone prepares the prompt; Wan 2.2 S2V performs heavy generation on its free public interface.</p>
    <Note><Readout label="Engine" value={wan.name} /><Readout label="Cost" value="Free public route" /><Readout label="API key" value="None" /></Note>
    <textarea value={prompt} readOnly rows={8} className="w-full rounded-xl border border-border bg-background/60 p-3 text-xs" />
    <StudioButton className="w-full" onClick={() => void open()}>Copy prompt & open free video engine</StudioButton>
  </Panel>;
}
