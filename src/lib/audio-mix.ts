...
export async function mixVocalsWithInstrumental(vocal: Blob | string, instrumental: Blob | string) {
  ...
  try {
    ...
    vocalSource.buffer = vocalBuffer; instrumentalSource.buffer = instrumentalBuffer;
    const vocalGain = offline.createGain(); vocalGain.gain.value = 0.96;
    const instrumentalGain = offline.createGain(); instrumentalGain.gain.value = 0.92;
    ...
    vocalSource.start(0); instrumentalSource.start(0);
