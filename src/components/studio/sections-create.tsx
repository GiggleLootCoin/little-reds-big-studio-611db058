...
export function LabPanel() {
  ...
  return (
    <Panel eyebrow="Audio lab" title="Red'sLab Stem Studio" icon={<Scissors className="size-5" />}>
      ...
      <StudioButton
        className="w-full"
        onClick={() => demucs && window.open(demucs.url, "_blank", "noopener,noreferrer")}
        disabled={!demucs}
      >
...
export function VideoPanel() {
  ...
  return (
    <Panel eyebrow="Video" title="Free Video Generation" icon={<Film className="size-5" />}>
      <p className="text-sm text-muted-foreground">
        Your phone prepares the prompt; LTX 2.3 performs the heavy generation on free public GPU
        capacity. The Studio's main Create panel can run the live endpoint directly.
      </p>
      <Note>
        <Readout label="Engine" value={ltx?.name ?? "LTX 2.3"} />
      ...
      />
      <StudioButton className="w-full" onClick={() => void open()} disabled={!ltx}>
