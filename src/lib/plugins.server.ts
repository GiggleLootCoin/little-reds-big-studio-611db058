 ...
 *
 * Core creative models are open-source/open-weight and require no API keys.
 * Heavy models execute through a free local, Colab, or browser runner.
 * No commercial inference provider is part of this runtime.
 */
...
/** Open models never have secrets. */
...
export function extractMedia(output: unknown): string[] {
  ...
  const walk = (value: unknown, depth = 0) => {
    ...
    if (typeof value === "object") Object.values(value as Record<string, unknown>).forEach((v) => walk(v, depth + 1));
  };
  walk(output);
