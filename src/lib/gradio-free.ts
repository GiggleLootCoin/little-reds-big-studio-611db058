...
const ROUTES: Record<string, RouteCandidate[]> = {
  ...
  voiceClone: [
    { space: "Qwen/Qwen3-TTS", endpoints: ["/generate_voice_clone", "/generate_custom_voice"], priority: 180 },
    { space: "multimodalart/higgs-audio-v3-tts", endpoints: ["/synthesize"], priority: 160 },
  ],
  voicePreset: [
    { space: "Qwen/Qwen3-TTS", endpoints: ["/generate_custom_voice", "/generate_speech"], priority: 180 },
...
function adaptInputs(logicalId: string, space: string, endpoint: string, inputs: Record<string, unknown> | unknown[]) {
  if (logicalId === "speechToText") {
    ...
    return endpoint === "/transcribe" ? { inputs: audio, task: "transcribe" } : { inputs: audio, task: "transcribe" };
  }
