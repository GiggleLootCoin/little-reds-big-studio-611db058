...
type HordeJob = { id: string; message?: string; done?: boolean; faulted?: boolean; generations?: Array<{ img?: string; text?: string }> };

async function request(path: string, init?: RequestInit) {
  const response = await fetch(`${HORDE}${path}`, { ...init, headers: { apikey: ANONYMOUS_KEY, "Content-Type": "application/json", ...(init?.headers ?? {}) } });
...
export async function hordeImage(prompt: string) {
  const job = await request("/generate/async", { method: "POST", body: JSON.stringify({ prompt, params: { width: 768, height: 768, steps: 20, n: 1 } }) }) as { id: string };
...
export async function hordeText(prompt: string) {
  const job = await request("/generate/async", { method: "POST", body: JSON.stringify({ prompt, params: { max_length: 512, max_context_length: 4096 } }) }) as { id: string };
  const result = await poll(job.id);
