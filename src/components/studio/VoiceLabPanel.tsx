...
export function VoiceLabPanel() {
  ...
  const preview = async () => {
    setBusy(true);
    setAudioUrl(null);
    setStatus("Connecting directly to Qwen3-TTS…");
    try {
  ...
  const clone = async () => {
    ...
    setBusy(true);
    setAudioUrl(null);
    setStatus("Creating a real voice-clone preview…");
    try {
  ...
  return (
    <Panel eyebrow="VOICE LAB" title="Buddy's voice" icon={<Mic2 className="size-5" />} defaultOpen>
      ...
      <Note>
        Buddy keeps your reference voice in this browser's local storage. The selected free public
        engine receives the sample only for that generation request. Only use voice material you own
        or are authorized to submit.
      </Note>
    </Panel>
  );
}
